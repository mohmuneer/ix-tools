  ...
  if (done) {
    return (
      <div dir="rtl" lang="ar" style={{ position: 'relative', minHeight: '100vh', width: '100%', overflow: 'hidden', background: '#020617' }}>
        <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, height: '64px', display: 'flex', alignItems: 'center', padding: '0 24px', direction: 'rtl', background: 'rgba(10,15,35,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {config.logo?.logoUrl ? (
          ...
          <div style={{ minWidth: 0, marginRight: '12px', textAlign: 'right' }}>
            <h1 style={{ fontSize: '14px', fontWeight: 700, color: 'white', fontFamily: FONT, margin: 0, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{config.logo?.systemName || 'Onyx IX'}</h1>
            <p style={{ fontSize: '10px', color: '#64748b', fontFamily: FONT, margin: 0, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{config.logo?.companyName || 'Ultimate Solutions'}</p>
          </div>
        </header>
